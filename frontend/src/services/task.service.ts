...
import { Task, Deadline, CreateTaskInput, CreateDeadlineInput, TaskFilters, AnalyticsData, WeeklyProgressResponse, HeatmapData } from '@/types/task.types';
...
export const analyticsService = {
  ...
  async getWeeklyProgress(): Promise<WeeklyProgressResponse> {
    const response = await api.get<ApiResponse<WeeklyProgressResponse>>('/analytics/weekly-progress');
    return response.data.data!;
  },

  async getHeatmap(days: number = 60): Promise<HeatmapData[]> {
    const response = await api.get<ApiResponse<HeatmapData[]>>('/analytics/heatmap', { params: { days } });
    return response.data.data || [];
  },
};
